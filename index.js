// ==================================================================================
//  🟢 GREEN CHIP V6 "GOD MODE" - COMPACT EDITION
...
//  Updates: Removed "Dead Space" in Embeds for cleaner, tighter calls.
// ==================================================================================
...
// 🖌️ EMBED BUILDER - COMPACT / NO DEAD SPACE EDITION
async function sendAlert(pair, analysis, source) {
    ...
    if (source === 'PROFILE') { badge = '💎'; color = '#00D4FF'; } // Blue
    if (analysis.status === 'GRADUATED') { badge = '🎓'; color = '#00FF00'; }

    const links = socials.map(s => `[${s.type.toUpperCase()}](${s.url})`).join(' • ') || '⚠️ No Socials';

    // 🟢 COMPACT METRICS LINE (Replaces vertical list)
    const metricsLine = `**MC:** \`${Utils.formatUSD(analysis.fdv)}\` • **Liq:** \`${Utils.formatUSD(analysis.liq)}\` • **Vol:** \`${Utils.formatUSD(analysis.vol)}\` • **Age:** \`${Utils.getAge(pair.pairCreatedAt)}\``;
...
**Score:** ${analysis.hype}/100 (${analysis.hype > 40 ? "🔥 High" : "✅ Steady"})

[**🛒 BUY ON GMGN**](${CONFIG.URLS.REFERRAL}) | [**📈 Dex**](${dexLink}) | [**⚡ Photon**](${photonLink})
`)
        .setThumbnail(pair.info?.imageUrl || 'https://cdn.discordapp.com/embed/avatars/0.png')
        .setFooter({ text: `Green Chip V6 • ${new Date().toLocaleTimeString()}`, iconURL: client.user.displayAvatarURL() });
...
client.once('ready', () => {
    ...
    client.user.setActivity('Solana Markets | V6 Compact', { type: ActivityType.Competing });
});
